...
const addUserNotification = async (req, res) => {
  try {
    ...
    let orderNo = await userNotificationModel
      .find({})
      .limit(1)
      .sort({ order_no: -1 });
...
const getUserWiseNotifications = async (req, res) => {
  try {
    ...
    let result = await userNotificationLogModel.aggregate([
      {
        $match: {
          $and: [
            { user_id: req.user._id, creationAt: { $gte: new Date(date) } },
          ],
        },
      },
      {
    ...
    const updateUserNotificationLogModel =
      await userNotificationLogModel.updateMany(
        { user_id: req.user._id, isRead: false },
        { $set: userNotificationLogObj }
      );

    if (updateUserNotificationLogModel) {
      //let localURL = "http://localhost:3001/notifications/api/update/notifications";
      let liveURL = "http://app.zenoteit.com/notifications/api/update/notifications";
