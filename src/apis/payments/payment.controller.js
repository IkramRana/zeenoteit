...
const startFreeTrial = async (req, res) => {
    try {
        ...
        return res.status(200).json({ status: true,message: 'Free trial started.', token: token, plan_expiry: moment().add(14,"days").toDate(),
        plan_identifier: "trial" });
...
const validatePayment = async (req, res) => {
    try {
        ...
        return res.status(200).json({ status: true,message: 'Subscription started.', token: token,plan_expiry: moment().add(30,"days").toDate(),
        plan_identifier: "paid.gold" });
